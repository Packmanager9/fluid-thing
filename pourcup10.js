...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    class Water {
        ...
        move(){
            this.body.ymom += ((Math.cos(position) * .45)*(Math.max((7-this.links.length),.1)*.5))*gravity.value
            this.body.xmom += ((Math.sin(position) * .45)*(Math.max((7-this.links.length),.1)*.5))*gravity.value
