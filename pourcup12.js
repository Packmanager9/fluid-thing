...
if(window.DeviceMotionEvent){
  ...
  function motion(event){
    ...
    position = event.accelerationIncludingGravity.x
    // position += event.accelerationIncludingGravity.z
    // position += event.accelerationIncludingGravity.y
  }
...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    class Water {
        ...
        move(){
            this.body.ymom += ((Math.cos(position) * .45)*(Math.max(( (linklength.value+1) -this.links.length),.1)*.5))*gravity.value
            this.body.xmom += ((Math.sin(position) * .45)*(Math.max(( (linklength.value+1) -this.links.length),.1)*.5))*gravity.value
    ...
    function main() {
        ...
        if(cup.drops.length < drops.value){
            cup.drops.push(new Water(cup.center.x, cup.center.y))
        }
    }
})