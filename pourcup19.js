...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    class Water {
        ...
        move(){
            this.body.ymom += ((Math.cos(positiony) * .45)*(Math.max(( (linklength.value+1) -this.links.length),.1)*.5))*gravity.value
            this.body.xmom += ((Math.sin(positionx) * .45)*(Math.max(( (linklength.value+1) -this.links.length),.1)*.5))*gravity.value
