...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    class Water {
        ...
        move(){
            this.body.ymom += ((((positiony) * .45)*(Math.max(( (linklength.value+1) -this.links.length),.1)*.5))*gravity.value)*.4
            this.body.xmom += ((((positionx) * .45)*(Math.max(( (linklength.value+1) -this.links.length),.1)*.5))*gravity.value)*.4
